...
$(function () {
    $("#button3").dxButton({
        ...
        onClick: async function () {
            ...
            for (const i in guidsDemouldedElements) {
                debugInfo = debugInfo.concat("<br />guid demoulded element: " + guidsDemouldedElements[i]);
                $(debug).html(debugInfo); 
                var demouldedObjects = await API.viewer.getObjects({ parameter: { properties: { ["Default.GUID"]: guidsDemouldedElements[i] } } });
                for (const j in demouldedObjects) {
                    debugInfo = debugInfo.concat("<br />type of j: " + typeof j);
                    debugInfo = debugInfo.concat("<br />type of demouldedObject[j]: " + typeof demouldedObjects[j]);
                    $(debug).html(debugInfo); 
                    demouldedObjects.push(demouldedObjects[j]);
                }
            }
