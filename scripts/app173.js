...
$(function () {
    $("#button3").dxButton({
        ...
        onClick: async function () {
            ...
            for (const guidsDemouldedElement of guidsDemouldedElements) {
                debugInfo = debugInfo.concat("<br />guid demoulded element: " + guidsDemouldedElement);
                $(debug).html(debugInfo); 
                var demouldedObjects = await API.viewer.getObjects({ parameter: { properties: { ["Default.GUID"]: guidsDemouldedElement } } });
                for (const demouldedObject of demouldedObjects) {
                    ...
                    $(debug).html(debugInfo); 
                    demouldedObjects.push(demouldedObject);
                }
            }
