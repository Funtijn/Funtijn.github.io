...
$(function () {
    $("#button").dxButton({
        ...
        type: "success",
        onClick: function () {
            DevExpress.ui.notify("Filter wordt toegepast.");
            setObjectsByProp2("Default.MERKPREFIX", "K");
        },
    });
});
...
async function setObjectsByProp2(propNameFilter, propValueFilter) {
    SetText("setObjectsByProp2");
    await API.viewer.setSelection(getPropSelector(propNameFilter, propValueFilter), "set");
}
...
function getPropSelector(propNameFilter, propValueFilter) {
    return {
        parameter: {
            properties: {
                [$(propNameFilter).val()]: $(propValueFilter).val()
